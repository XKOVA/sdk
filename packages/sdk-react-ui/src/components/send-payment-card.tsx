 ...
 */
export function SendPaymentCard({
  allowedTokens,
  ...
  const [selectedContract, setSelectedContract] = useState<string>(defaultTokenContract ?? (includeNative ? "native" : ""));
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const isAccountIdentifier = useCallback((value: string) => /^0x[a-fA-F0-9]{40}$/.test(value.trim()), []);
  const isEmail = useCallback((value: string) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.trim()), []);
  ...
  const tokenOptions = useMemo(() => {
    ...
    if (!allowedTokens) return tenantTokens;
  ...
  const selectedTokenMeta = useMemo(() => {
    if (selectedContract === "native") return null;
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      const networkId = String((network as any)?.networkId ?? (network as any)?.network_id ?? "");
      if (!networkId) {
        throw new Error("No network configured");
      }

      const resolveContactType = (contact: string, provided?: string) => {
      ...
      const submitPayload = {
        transactionType: "p2p_send" as const,
        amountWei,
        networkId,
      ...
      const resolvedRecipientWallet =
        typeof (resolvedPayload as any)?.recipient_wallet_address === "string"
          ? String((resolvedPayload as any).recipient_wallet_address).trim()
          : typeof (resolvedPayload as any)?.recipientWalletAddress === "string"
            ? String((resolvedPayload as any).recipientWalletAddress).trim()
            : null;
      const resolvedIsPending =
        typeof (resolvedPayload as any)?.is_pending_payment === "boolean"
          ? Boolean((resolvedPayload as any).is_pending_payment)
          : typeof (resolvedPayload as any)?.isPendingPayment === "boolean"
            ? Boolean((resolvedPayload as any).isPendingPayment)
            : null;
      const resolvedExpiresAt =
        typeof (resolvedPayload as any)?.expires_at === "string"
          ? String((resolvedPayload as any).expires_at).trim()
          : typeof (resolvedPayload as any)?.expiresAt === "string"
            ? String((resolvedPayload as any).expiresAt).trim()
            : null;
      ...
      const finalPayload = {
        ...
        isPendingPayment:
          typeof resolvedIsPending === "boolean" ? resolvedIsPending : submitPayload.isPendingPayment,
      ...
      await submitSendPayment(
        { ...finalPayload, transactionHash: resolvedTxHash.trim() },
      ...
      <Card>
        <CardHeader>
          ...
          <CardTitle className="flex items-center gap-2">
            ...
            description={<CardDescription>Send tokens to another account or email.</CardDescription>}
          />
        </CardHeader>
        <CardContent>
          <CardEmptyState>No network configured for this tenant.</CardEmptyState>
        </CardContent>
      </Card>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="recipient"
              type="text"
              placeholder="0x... or recipient@example.com"
              value={recipient}
              onChange={(e) => setRecipient(e.target.value)}
              disabled={isSubmitting || !senderAccount}
            />
          </div>

          <div className={showTokenSelect ? "grid grid-cols-1 sm:grid-cols-2 gap-3" : "space-y-2"}>
            <div className="space-y-2">
            ...
            {showTokenSelect ? (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  disabled={isSubmitting || !senderAccount}
                >
                  {includeNative && <option value="native">{network.symbol ?? "Native"}</option>}
                  {selectableTokenOptions.map((token) => (
          ...
          <Button
            type="submit"
            className="w-full"
            disabled={isSubmitting || !senderAccount || !recipient || !amount}
          >
            {isSubmitting ? (
              <>
