...
export function TenantCard() {
  ...
  const environment = useMemo(() => {
    ...
    if (primaryNetwork?.isTestnet === true) return "test";
    if (primaryNetwork) return "live";
    return null;
      ...
      <Card>
        <CardHeader>
          <CardHeaderRow
            ...
            description={<CardDescription>Tenant configuration.</CardDescription>}
          />
        </CardHeader>
        <CardContent>
          <CardEmptyState>Sign in to view your tenant configuration.</CardEmptyState>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader>
          <CardHeaderRow
            ...
            description={<CardDescription>Tenant identity and configuration.</CardDescription>}
            actions={
              <div className="flex items-center gap-2">
                ...
                {environment ? (
                  <Badge
                    variant={environment === "test" ? "warn" : "success"}
                    className="flex items-center gap-1"
                  >
                    {environment === "test" ? <TestTube className="h-3 w-3" /> : <Globe className="h-3 w-3" />}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div className="space-y-1">
            <CardSectionLabel>Tenant Name</CardSectionLabel>
            <div className="text-sm font-medium">{tenant.name ?? "-"}</div>
          </div>
          <div className="space-y-1">
            <CardSectionLabel>Tenant Slug</CardSectionLabel>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <CardSectionLabel>Tenant ID</CardSectionLabel>
            <Tooltip>
              ...
              <TooltipContent>{copied ? "Copied" : "Copy tenant ID"}</TooltipContent>
            </Tooltip>
          </div>
          <code className="block w-full rounded-lg bg-muted px-3 py-2 text-xs font-mono break-all">
            {tenant.id}
          </code>
        </div>

        <div className="space-y-1">
          <CardSectionLabel>Network Name</CardSectionLabel>
          <div className="text-sm font-medium">{primaryNetwork?.name ?? "-"}</div>
        </div>

        <div className="space-y-2">
          <CardSectionLabel>Tenant Tokens</CardSectionLabel>
          {erc20Tokens.length === 0 ? (
            <div className="text-sm text-muted-foreground">No ERC-20 tokens configured.</div>
          ) : (
            <div className="flex flex-wrap gap-2">
              {erc20Tokens.map((token) => (
                <Tooltip key={`${token.networkId}:${token.contract}:${token.symbol}`}>
                  <TooltipTrigger asChild>
                    <div className="flex items-center gap-2 rounded-full border border-border bg-background px-3 py-1.5 text-sm cursor-help">
                  ...
                  <TooltipContent className="max-w-[260px] break-all font-mono">
