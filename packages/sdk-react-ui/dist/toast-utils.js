...
import { generateRequestId } from "@xkova/sdk-core/telemetry";
let uiErrorTelemetryHandler = null;
/**
 ...
 */
export function setUIErrorTelemetryHandler(handler) {
 ...
 */
export const createUIErrorTelemetryAdapter = (telemetry, opts) => {
...
const emitErrorTelemetry = (context, message) => {
    if (!uiErrorTelemetryHandler)
        return;
 ...
 */
export function getUserFriendlyErrorMessage(err, fallback) {
    ...
    const technical = /^(authorization failed|user verification failed|fetch failed|networkerror|typeerror:|tx_|http\s?\d{3}|internal server error|invalid response|unexpected token)/i.test(m) ||
        /(bearer\s+[a-z0-9\-_.=]+|access_token|authorization_token|operation_token|refresh_token)/i.test(m) ||
...
export function logError(context, err, extra) {
    const safeMessage = getUserFriendlyErrorMessage(err, "An unexpected error occurred.");
 ...
 */
export function toastError(context, err, fallback) {
...
export function notify(kind, message, opts) {
    ...
    if (kind === "error") {
        const safeMessage = error
            ...
            : getUserFriendlyErrorMessage(new Error(message), "An unexpected error occurred.");
