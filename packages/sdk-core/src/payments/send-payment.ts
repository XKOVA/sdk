...
const resolveContactType = (
  ...
  if (provided) {
    ...
    if (!normalized) {
      throw new Error(
        "contactType must be one of: account, email, phone, username",
      );
    }
    return normalized;
  }
 ...
 */
export const normalizeSubmitSendPaymentInput = (
  payload: SubmitSendPaymentInput,
): { normalized: NormalizedSendPaymentSubmission; receiptPayload: SendPaymentReceiptPayload } => {
  ...
  const recipientContact = String(
    payload.recipientContact ?? payload.recipientEmail ?? "",
  ).trim();
  if (!recipientContact) {
  ...
  const isPending =
    payload.isPendingPayment !== undefined
  ...
  const normalized: NormalizedSendPaymentSubmission = {
    transactionType,
    amountWei,
    networkId,
    tokenAddress,
  ...
  const receiptPayload: SendPaymentReceiptPayload = {
...
export const buildSubmitSendPaymentBody = (params: {
  ...
  const approvalTxHash = String(
    approval.transactionHash ?? approval.userOpHash ?? "",
  ).trim();
  ...
  const resolvedRecipientContact =
    typeof (resolvedPayload as any).recipient_contact === "string"
      ? (resolvedPayload as any).recipient_contact
      : normalized.recipientContact;
  const resolvedContactType =
    typeof (resolvedPayload as any).contact_type === "string"
      ? (resolvedPayload as any).contact_type
      : normalized.contactType;
  const resolvedRecipientWallet =
    typeof (resolvedPayload as any).recipient_wallet_address === "string"
      ? (resolvedPayload as any).recipient_wallet_address
      : normalized.recipientWallet;
  const resolvedIsPending =
    typeof (resolvedPayload as any).is_pending_payment === "boolean"
      ? (resolvedPayload as any).is_pending_payment
      : normalized.isPending;
  const resolvedFingerprint =
    typeof (resolvedPayload as any).fingerprint === "string"
      ? (resolvedPayload as any).fingerprint
      : normalized.fingerprint;
  const resolvedExpiresAt =
    typeof (resolvedPayload as any).expires_at === "string"
      ? (resolvedPayload as any).expires_at
      : normalized.expiresAt;
  const resolvedRecipientProfileId =
    typeof (resolvedPayload as any).recipient_profile_id === "string"
      ? (resolvedPayload as any).recipient_profile_id
      : input.recipientProfileId;
  const resolvedRecipientName =
    typeof (resolvedPayload as any).recipient_name === "string"
      ? (resolvedPayload as any).recipient_name
      : input.recipientName;
  ...
  const normalizedRecipientProfileId = normalizeOptionalString(
