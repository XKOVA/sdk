...
export type IeeReceiptProviderResult =
  | IeeReceiptProviderApproved
  | { status: "cancelled" }
...
const SERVER_REQUIRED_KEYS: Partial<Record<CanonicalServerActionType, string[]>> = {
...
const normalizeContactType = (value: string): string | null => {
  const normalized = value.trim().toLowerCase();
  if (CONTACT_TYPES.has(normalized)) return normalized;
  return null;
};

const requireString = (payload: Record<string, unknown>, key: string, actionType: string) => {
...
const requireBoolean = (payload: Record<string, unknown>, key: string, actionType: string) => {
  const value = payload?.[key];
...
const validateSendPaymentSubmit = (payload: Record<string, unknown>) => {
  ...
  if (!normalizedContactType) {
    throw new ValidationError(
      `Invalid contact_type for action_type '${actionType}' (expected account, email, phone, or username)`,
    );
  }
...
const EXPOSED_ACTION_SPECS: Array<Pick<IeeActionSpec, "sdkActionType" | "serverActionType">> = [
...
const THIRD_PARTY_UNSUPPORTED_SERVER_ACTION_TYPES = new Set<CanonicalServerActionType>([]);

const ACTION_SPECS: IeeActionSpec[] = EXPOSED_ACTION_SPECS.map((spec) => {
  const requiredKeys = SERVER_REQUIRED_KEYS[spec.serverActionType] ?? [];
  const validate =
    spec.serverActionType === "send_payment_submit_v1" ? validateSendPaymentSubmit : undefined;
  return {
    ...
    validate,
    requiresIee: true,
    thirdPartyUnsupported: THIRD_PARTY_UNSUPPORTED_SERVER_ACTION_TYPES.has(
      spec.serverActionType,
    ),
  };
});

const ACTION_SPECS_BY_SDK = new Map<string, IeeActionSpec>(
  ACTION_SPECS.map((spec) => [spec.sdkActionType, spec]),
);
const ACTION_SPECS_BY_SERVER = new Map<string, IeeActionSpec>(
  ACTION_SPECS.map((spec) => [spec.serverActionType, spec]),
);

export const resolveIeeActionSpec = (actionType: string): IeeActionSpec | null => {
...
const normalizeSdkActionType = (actionType: string): string => {
...
export const prepareIeeActionPayload = (params: {
  ...
  payload: Record<string, unknown>;
  context?: IeeActionContext | null;
}): { spec: IeeActionSpec; payload: Record<string, unknown> } => {
  const spec = resolveIeeActionSpec(params.actionType);
  if (!spec) {
    throw new IeeError(
      `SafeApprove action mapping missing for '${params.actionType}'`,
      "IEE_ACTION_MAPPING_MISSING",
      {
...
export class IeeOrchestrator {
  private readonly receiptProvider?: IeeReceiptProvider;
  private readonly requireExplicitReceipt: boolean;
  private readonly contextProvider?: () => IeeActionContext | null | undefined;
  private readonly inflight: Map<string, Promise<IeeReceiptApproval>> = new Map();
  ...
  async ensureReceipt(params: {
    ...
    if (spec.thirdPartyUnsupported) {
      throw new IeeError(
        `Action is not supported for third-party clients (${spec.sdkActionType}).`,
        "THIRD_PARTY_ACTION_UNSUPPORTED",
        { sdkActionType: spec.sdkActionType, serverActionType: spec.serverActionType },
      );
    }
    ...
    if (this.requireExplicitReceipt || !this.receiptProvider) {
      throw new IeeError(
        `SafeApprove receipt required for ${spec.sdkActionType}. Run in a browser with a SafeApprove provider or pass a receipt explicitly.`,
        "IEE_REQUIRED",
        { sdkActionType: spec.sdkActionType, serverActionType: spec.serverActionType },
      );
    }
    ...
    if (!normalizeContextValue(payload["tenant_id"])) missingContext.push("tenant_id");
    if (!normalizeContextValue(payload["client_id"])) missingContext.push("client_id");
    if (!normalizeContextValue(payload["user_id"])) missingContext.push("user_id");
    if (missingContext.length > 0) {
      throw new ValidationError(
        `Missing SafeApprove context: ${missingContext.join(", ")}.`,
      );
    }
    ...
    const promise = this.receiptProvider
      ...
      .then((result) => {
        ...
        if (result.status === "cancelled") {
          throw new IeeError(
            `SafeApprove approval cancelled for ${spec.sdkActionType}.`,
            "IEE_CANCELLED",
            { sdkActionType: spec.sdkActionType, serverActionType: spec.serverActionType },
          );
        }
        throw new IeeError(
          result.error?.message || `SafeApprove approval failed for ${spec.sdkActionType}.`,
          "IEE_FAILED",
          {
            sdkActionType: spec.sdkActionType,
            serverActionType: spec.serverActionType,
