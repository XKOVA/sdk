import {
  ...
  generateRequestId,
  type SDKTelemetry,
  type SDKTelemetryError,
  type SDKTelemetryRedactor,
} from "./telemetry.js";
 ...
 */
export const createHeadlessErrorTelemetryAdapter = (
...
export function createConsoleTelemetry(opts?: {
  ...
  const logger = opts?.logger ?? (console as any);
  ...
  const pick = (kind: "debug" | "info" | "warn" | "error") => {
    return (
      (logger as any)[kind] ??
      (logger as any).log ??
      (() => {
  ...
  return {
    ...
    onRequestError: (e: SDKTelemetryError) => {
      if (!logErrors) return;
      const payload: any = {
        requestId: e.requestId,
        method: e.method,
  ...
  return {
    ...
    onRequestError: (e) => {
      ...
      if (span) {
        ...
        if (typeof e.status === "number") span?.setAttribute?.("http.status_code", e.status);
        span?.recordException?.(e.error);
        span?.setStatus?.({ code: 2, message: (e as any)?.error?.message });
        span?.end?.();
        spans.delete(e.requestId);
      }
    },
...
export function createDatadogTraceTelemetry(opts: {
  tracer: {
    startSpan: (
      name: string,
      options?: { service?: string; resource?: string; type?: string; tags?: Record<string, any> },
    ) => {
  ...
  return {
    ...
    onRequestRetry: (e) => {
      ...
      span?.setTag?.(
        "xkova.retry.reason",
        e.reason.kind === "status" ? `status:${e.reason.status}` : "network",
      );
    },
    onRequestSuccess: (e) => {
      const span = spans.get(e.requestId);
    ...
    onRequestError: (e) => {
      const span = spans.get(e.requestId);
      if (span) {
        if (typeof e.status === "number") span?.setTag?.("http.status_code", e.status);
        span?.setTag?.("error", true);
        span?.setTag?.("xkova.duration_ms", e.durationMs);
        const err: any = e.error;
        if (err?.message) span?.setTag?.("error.message", err.message);
        if (err?.name) span?.setTag?.("error.type", err.name);
        span?.finish?.();
        spans.delete(e.requestId);
      }
    },
...
export function createSentryTelemetry(opts: {
  Sentry: {
    ...
    startSpan?: (ctx: any) => { setAttribute?: (k: string, v: any) => void; setStatus?: (s: any) => void; end?: () => void };
    startSpanManual?: (ctx: any) => { setAttribute?: (k: string, v: any) => void; setStatus?: (s: any) => void; end?: () => void };
  };
  spanName?: string;
  redact?: SDKTelemetryRedactor;
  /**
  ...
  return {
    ...
    onRequestStart: (e) => {
      ...
      if (span) spans.set(e.requestId, span);
    },
    onRequestRetry: (e) => {
      const span = spans.get(e.requestId);
