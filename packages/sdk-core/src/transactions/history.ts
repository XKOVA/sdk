import {
  ...
  type TransactionStatusCanonical,
  type TransactionImageSource,
  type TokenAsset,
  type TransferProvider,
} from "../types.js";
import { formatTransactionAmount, type TransactionHistoryParams } from "../services.js";
 ...
 */
export const deriveFeedStatus = (
  statusCanonical: TransactionStatusCanonical | null | undefined,
): "pending" | "completed" | "failed" => {
  if (statusCanonical === "success") return "completed";
  if (statusCanonical === "failed") return "failed";
  return "pending";
};
 ...
 */
export const buildTransactionHistorySearchParams = (
  params?: TransactionHistoryParams & { view?: TransactionHistoryView },
): URLSearchParams => {
  ...
  const excludeUserOperationWrappers =
    typeof params?.excludeUserOperationWrappers === "boolean"
      ...
      : true;
  if (params?.account) search.set("account", params.account.toLowerCase());
  if (params?.agentInstallationId) search.set("agentInstallationId", params.agentInstallationId);
  if (params?.agentServiceId) search.set("agentServiceId", params.agentServiceId);
  if (params?.networkId !== undefined) search.set("networkId", String(params.networkId));
  if (params?.eventType) search.set("eventType", params.eventType);
  if (params?.eventSubtype) search.set("eventSubtype", params.eventSubtype);
  if (params?.executionMethod) search.set("executionMethod", params.executionMethod);
  if (excludeUserOperationWrappers) {
    search.set("excludeUserOperationWrappers", "true");
  }
  if (params?.status) search.set("status", params.status);
  if (params?.direction) search.set("direction", params.direction);
  if (params?.contract) search.set("contract", params.contract.toLowerCase());
  ...
  if (typeof params?.offset === "number" && !params?.cursor) search.set("offset", String(params.offset));
  return search;
};
 ...
 */
export const normalizeTransactionHistoryResponse = (
  payload: TransactionHistoryResponse,
  options?: {
    tokens?: Array<TokenAsset | string>;
    view?: TransactionHistoryView;
    transferProviders?: TransferProvider[];
  },
): TransactionHistoryResponse & { transactions: TransactionHistoryDisplayItem[] } => {
  const tokens = options?.tokens ?? [];
  const transferProviders = options?.transferProviders ?? [];
  const faucetByAddress = new Map<
    string,
    { providerId?: string; name?: string; id?: string; logoUrl?: string | null }
  >();
  const providerById = new Map<string, { logoUrl?: string | null; name?: string }>();
  const providerByProviderId = new Map<string, { logoUrl?: string | null; name?: string }>();
  ...
  const categoryLabel = (category: TransactionHistoryItem["category"] | undefined): string | null => {
    switch (category) {
      case "agent":
        return "Agent";
      case "p2p":
  ...
  const eventTypeLabel = (eventType?: string | null): string | null => {
    switch (eventType) {
      case "token_transfer":
        return "Transfer";
      case "nft_transfer":
  ...
  const normalizeDirection = (d: any): TransactionDirection => {
    if (d === "incoming") return "in";
    if (d === "outgoing") return "out";
    if (d === "in" || d === "out" || d === "internal") return d;
    return (d as any) ?? "internal";
  };

  const isImageSource = (value: unknown): value is TransactionImageSource =>
    value === "ramp_provider" ||
    value === "agent_icon" ||
  ...
  const resolveExpectedImageSource = (
    tx: TransactionHistoryItem,
  ): TransactionImageSource => {
    if (tx.category === "transfer") return "ramp_provider";
    if (tx.category === "agent") return "agent_icon";
    if (tx.category === "p2p") return "counterparty_avatar";
    if (tx.provenance === "external") return "network_logo";
    return "fallback";
  };

  const resolveCanonicalImage = (params: {
    tx: TransactionHistoryItem;
    expectedSource: TransactionImageSource;
    counterpartyLabel: string | null;
    providerLabel: string | null;
    providerLogoUrl: string | null;
    legacyCounterpartyAvatarUrl: string | null;
    legacyAgentServiceIconUrl: string | null;
  }): TransactionHistoryItem["image"] => {
    const {
      tx,
      expectedSource,
      counterpartyLabel,
    ...
    const apiImageRaw = (tx as any)?.image;
    const apiImage =
      apiImageRaw && typeof apiImageRaw === "object" && !Array.isArray(apiImageRaw)
        ? (apiImageRaw as Record<string, unknown>)
        : null;
    const apiUrl =
      typeof apiImage?.url === "string" && apiImage.url.trim().length > 0
        ? apiImage.url.trim()
        : null;
    const apiSource = isImageSource(apiImage?.source) ? apiImage.source : null;
    const apiAlt =
      typeof apiImage?.alt === "string" && apiImage.alt.trim().length > 0
        ? apiImage.alt.trim()
        : null;
  ...
  const mappedTransactions: TransactionHistoryDisplayItem[] = (payload.transactions ?? []).map(
    (tx: TransactionHistoryItem) => {
      const txWithLegacy = tx as TransactionHistoryItem & {
        counterpartyAvatarUrl?: string | null;
        rampProviderLogoUrl?: string | null;
        agentServiceIconUrl?: string | null;
      };
      const {
        counterpartyAvatarUrl: legacyCounterpartyAvatarUrl,
        rampProviderLogoUrl: legacyRampProviderLogoUrl,
        agentServiceIconUrl: legacyAgentServiceIconUrl,
        ...txBase
      } = txWithLegacy;
      const metadata = normalizeMetadata((tx as any)?.metadata);
      const agentServiceName =
        typeof tx?.agentServiceName === "string" &&
        tx.agentServiceName.trim().length > 0
          ? tx.agentServiceName.trim()
          : null;
      const isAgentTransaction =
        tx?.category === "agent" ||
        tx?.counterparty?.type === "agent" ||
        Boolean(tx?.agentServiceId) ||
        Boolean(agentServiceName);
      const shouldInjectAgentNote =
        isAgentTransaction &&
        agentServiceName &&
      ...
      const metadataWithAgentHint = shouldInjectAgentNote
        ...
        : metadata;
      const senderContact =
        readMetadataString(metadataWithAgentHint, ["sender_contact", "senderContact"]);
      const recipientContact =
        readMetadataString(metadataWithAgentHint, ["recipient_contact", "recipientContact"]);
      const rampProviderId =
        readMetadataString(metadataWithAgentHint, ["ramp_provider_id", "rampProviderId"]);
      const rampProviderName =
        readMetadataString(metadataWithAgentHint, ["ramp_provider_name", "rampProviderName"]);
      const faucetMatch = (() => {
        if (faucetByAddress.size === 0) return null;
      ...
      const rampLogoFromId = rampProviderId
        ...
        : null;
      const rampProviderLogoUrl =
        rampLogoFromProviderId ??
        rampLogoFromId ??
        faucetMatch?.logoUrl ??
      ...
      const tokenMeta = tx.contract
        ? tokens.find(
          (t): t is TokenAsset =>
            typeof t !== "string" && t.contract?.toLowerCase() === tx.contract?.toLowerCase(),
        )
        : undefined;

      const tokenDecimals = tx.tokenDecimals ?? tokenMeta?.decimals ?? 18;
      const tokenSymbol =
        tx.tokenSymbol ?? tokenMeta?.symbol ?? (tx.contract ? "TOKEN" : "NATIVE");
      const tokenLogoUrl =
        tokenMeta && "logoUrl" in tokenMeta ? (tokenMeta.logoUrl ?? null) : null;
      ...
      const displayAmount =
        tx.amountRaw != null && (tx.eventType === "token_transfer" || tx.eventType === "nft_transfer")
      ...
      const counterpartyLabel = (() => {
        ...
        if (isFaucet && providerLabel) return providerLabel;
      ...
      const image = resolveCanonicalImage({
        ...
        legacyCounterpartyAvatarUrl:
          typeof legacyCounterpartyAvatarUrl === "string"
            ? legacyCounterpartyAvatarUrl
            : null,
        legacyAgentServiceIconUrl:
          typeof legacyAgentServiceIconUrl === "string"
            ? legacyAgentServiceIconUrl
            : null,
      });
      ...
      return {
        ...
        image,
        rampProviderLogoUrl:
          image.source === "ramp_provider" ? image.url : null,
        displayType,
        counterpartyLabel,
        counterpartyAvatarUrl:
          image.source === "ramp_provider" ? null : image.url,
        senderContact,
        recipientContact,
  ...
  const filteredTransactions = (() => {
    if (options?.view === "events") return mappedTransactions;

    const hashesWithTransfers = new Set(
      mappedTransactions
        .filter((t: TransactionHistoryDisplayItem) => t.transactionHash && t.eventType === "token_transfer")
        .map((t: TransactionHistoryDisplayItem) => t.transactionHash as string),
    );

    return mappedTransactions.filter((t: TransactionHistoryDisplayItem) => {
      if (!t.transactionHash) return true;
      if (!hashesWithTransfers.has(t.transactionHash)) return true;
      return !(
        t.eventType === "contract_interaction" &&
