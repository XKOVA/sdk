...
import {
  ...
  AgentInstallationFailureCountResponse,
  AgentInstallationFailureCountsResponse,
  AgentInstallationFailuresResponse,
  AgentInstallQuestion,
  BootstrapPayload,
  BulkContactsOperationInput,
  BulkContactsOperationResult,
  ConfirmInstallationRequest,
  ConfirmInstallationResponse,
  Contact,
  ContactsListQuery,
  ...
  UserInfo,
  TransactionCategory,
  TransferTransactionsListResult,
  TransferTransactionsQuery,
  TransferTransaction,
  CreateTransferTransactionInput,
  ExecuteFaucetTransferInput,
  UpdateTransferTransactionInput,
  UpdateContactInput,
  UpdateInstallationConfigOptions,
} from './types.js';
...
class BaseService {
  ...
  protected async ensureIeeReceipt(params: {
    actionType: string;
...
export class AccountService extends BaseService {
  ...
  async getAccountState(): Promise<AccountState> {
    const raw = await this.client.get<any>('/account');
...
export class TenantConfigService extends BaseService {
  ...
  async getBootstrap(): Promise<BootstrapPayload> {
    const user = await this.client.get<any>('/oauth/user');
    const tenant = await this.client.get<any>('/oauth/tenant');
...
export class UserProfileService extends BaseService {
  ...
  async getProfile(): Promise<UserInfo> {
    const raw = await this.client.get<any>('/oauth/user');
  ...
  async updateProfile(
    ...
    const { receipt } = await this.ensureIeeReceipt({
      ...
      payload: {
        ...
        avatar_path:
          payload.avatar_path === undefined || payload.avatar_path === null
            ? ''
            : String(payload.avatar_path),
      },
      receipt: options?.receipt,
    });
    const raw = await this.client.put<typeof payload, any>(
      '/oauth/user',
      payload,
  ...
  async createAvatarUpload(
    ...
    const raw = await this.client.post<void, any>(
      '/oauth/user/avatar/upload',
      undefined,
      { headers: { 'X-XKOVA-IEE-Receipt': receipt } },
    );
    const ticket = {
  ...
  private async uploadToSignedUrl(
    ...
    if (typeof FormData !== 'undefined' && file instanceof FormData) {
      ...
      const blobInput: ArrayBuffer | Blob = (() => {
...
export class IeeService extends BaseService {
   ...
   */
  async createPrepTicket(
    input: CreateIeePrepTicketInput,
  ): Promise<IeePrepTicket> {
    ...
    const response = await this.client.post<
      typeof payload,
      { ticket_id: string; expires_at: string }
    >('/iee/tickets', payload);
...
export class SessionManagementService extends BaseService {
  ...
  async listSessions(options?: {
    ...
    if (typeof limit === 'number' && Number.isFinite(limit)) {
      ...
      if (normalized > 0) params.set('limit', String(normalized));
    }
    if (typeof offset === 'number' && Number.isFinite(offset)) {
      const normalized = Math.floor(offset);
      if (normalized >= 0) params.set('offset', String(normalized));
    }
    ...
    const raw = await this.client.get<any>(path);
    const container = (raw as any)?.data ?? raw;
    const sessionsRaw: unknown = (container as any)?.sessions;
     ...
     */
    const asDeviceType = (value: unknown): SessionDeviceType => {
     ...
     */
    const mapSession = (value: unknown): UserSession | null => {
    ...
    const sessions: UserSession[] = sessionsList
      .map(mapSession)
      .filter((s): s is UserSession => Boolean(s));

    const total =
      typeof (container as any)?.total === 'number'
        ? Number((container as any).total)
        : sessions.length;

    const currentSessionId =
      asNonEmptyString((container as any)?.currentSessionId) ?? null;

    const resolvedLimit =
      typeof (container as any)?.limit === 'number'
        ? Number((container as any).limit)
        : typeof limit === 'number' && Number.isFinite(limit)
          ? Math.floor(limit)
          : sessions.length;

    const resolvedOffset =
      typeof (container as any)?.offset === 'number'
        ? Number((container as any).offset)
  ...
  async revokeSession(
    ...
    const payload = await this.client.delete<any>(
      `/tenant/sessions/${encodeURIComponent(id)}`,
      { headers: { 'X-XKOVA-IEE-Receipt': receipt } },
    );
    ...
    return {
      ...
      revokedSessionId: String((payload as any)?.revokedSessionId ?? id),
    };
  }
  ...
  async revokeOtherSessions(options?: {
    ...
    if (!currentSessionId) {
      throw new ValidationError(
        'currentSessionId is required to revoke other sessions',
      );
    }
    const { receipt } = await this.ensureIeeReceipt({
    ...
    const payload = await this.client.delete<any>('/tenant/sessions/others', {
      headers: { 'X-XKOVA-IEE-Receipt': receipt },
    });
    return {
      success: Boolean((payload as any)?.success),
      message: String((payload as any)?.message ?? ''),
      revokedCount:
        typeof (payload as any)?.revokedCount === 'number'
          ? Number((payload as any).revokedCount)
          : 0,
      currentSessionId:
        typeof (payload as any)?.currentSessionId === 'string'
          ? (payload as any).currentSessionId
          : null,
    };
  }
}
...
export class ContactsService extends BaseService {
  ...
  async listContacts(
    ...
    if (typeof query.offset === 'number' && Number.isFinite(query.offset)) {
      ...
      if (normalized >= 0) search.set('offset', String(normalized));
    }

    const path = `/contacts${search.toString() ? `?${search.toString()}` : ''}`;
    return this.client.get<ContactsListResponse>(path);
  }
  ...
  async createContact(
    ...
    const payload: CreateContactInput = { email, name };
    const { receipt } = await this.ensureIeeReceipt({
    ...
    return this.client.post<CreateContactInput, Contact>('/contacts', payload, {
      headers: { 'X-XKOVA-IEE-Receipt': receipt },
    });
  }
  ...
  async updateContact(
    ...
    return this.client.patch<UpdateContactInput, Contact>(
  ...
  async deleteContact(
    ...
    return this.client.delete<DeleteContactResult>(
      `/contacts/${encodeURIComponent(id)}`,
      { headers: { 'X-XKOVA-IEE-Receipt': receipt } },
    );
  }
  ...
  async bulkContacts(
    ...
    if (
      ...
      throw new ValidationError(
        'operation must be one of: delete, favorite, unfavorite',
      );
    }

    const payload: BulkContactsOperationInput = {
      contactIds,
      operation: operation as BulkContactsOperationInput['operation'],
    };
...
export class TransfersService extends BaseService {
   ...
   */
  async listTransferTransactions(
    query: TransferTransactionsQuery = {},
  ): Promise<TransferTransactionsListResult> {
    const search = new URLSearchParams();
    if (query.type) search.set('type', query.type);
    if (query.status) search.set('status', query.status);
    if (query.providerId) search.set('providerId', query.providerId);
    if (query.networkId) search.set('networkId', query.networkId);
    if (query.cryptoSymbol) search.set('cryptoSymbol', query.cryptoSymbol);
    ...
    return this.client.get<TransferTransactionsListResult>(path);
  }
  ...
  async createTransferTransaction(
    ...
    const cryptoAmountWei =
      typeof input.cryptoAmountWei === 'string'
        ? input.cryptoAmountWei.trim()
        : '';
    if (!cryptoAmountWei) {
      throw new ValidationError(
        'cryptoAmountWei is required to create a transfer transaction',
      );
    }
    ...
    return this.client.post<typeof payload, TransferTransaction>(
  ...
  async executeFaucetTransfer(
    ...
    if (!cryptoAmountWei) {
      throw new ValidationError(
        'cryptoAmountWei is required to execute a faucet transfer',
      );
    }
    ...
    if (!resolvedTransactionHash) {
      throw new ValidationError(
        'transactionHash is required to execute a faucet transfer',
      );
    }
  ...
  async updateTransferTransaction(
    ...
    return this.client.patch<typeof payload, TransferTransaction>(
      `/transfers/transactions/${encodeURIComponent(id)}`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': receipt } },
    );
  }
...
export class SendPaymentsService extends BaseService {
   ...
   * @scope payments:read
   */
  async listSendPaymentHistory(
    query: SendPaymentsQuery = {},
  ): Promise<SendPaymentsListResult> {
    const search = new URLSearchParams();
    ...
    return this.client.get<SendPaymentsListResult>(path);
  }
   ...
   */
  async submitSendPayment(
    input: SubmitSendPaymentInput,
    options?: { receipt?: string },
  ): Promise<SendPayment> {
    const approval = await this.ensureIeeReceipt({
    ...
    return this.client.post<SubmitSendPaymentInput, SendPayment>(
      '/payments/send',
      input,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  ...
  async cancelSendPayment(
    ...
    return this.client.post<{}, PaymentActionResult>(
      `/payments/send/${encodeURIComponent(paymentId)}/cancel`,
      {},
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async remindSendPayment(
    ...
    return this.client.post<{}, PaymentActionResult>(
      `/payments/send/${encodeURIComponent(paymentId)}/remind`,
      {},
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
   ...
   */
  async verifySendPayment(
    paymentId: string,
    input: { transactionHash: string; network?: string },
    options?: { receipt?: string },
    ...
    return this.client.post<typeof input, TransactionVerificationResult>(
      `/payments/send/${encodeURIComponent(paymentId)}/verify`,
      input,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
   ...
   */
  async cancelPendingPaymentOnchain(
    paymentId: string,
    input: { cancelTxHash: string },
    options?: { receipt?: string },
  ): Promise<PaymentActionResult> {
    const approval = await this.ensureIeeReceipt({
    ...
    return this.client.post<typeof input, PaymentActionResult>(
      `/payments/send/${encodeURIComponent(paymentId)}/pending-payment/cancel`,
      input,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
...
export class PaymentRequestsService extends BaseService {
   ...
   * @scope payments:read
   */
  async listIncomingPaymentRequestHistory(
    ...
    return this.client.get<PaymentRequestsListResponse>(path);
  }
   ...
   * @scope payments:read
   */
  async listOutgoingPaymentRequestHistory(
    query: PaymentRequestsQuery = {},
  ): Promise<PaymentRequestsListResponse> {
    const search = new URLSearchParams();
    if (query.type) search.set('type', query.type);
  ...
  async createPaymentRequest(
    ...
    return this.client.post<typeof input, PaymentRequest>(
      '/payments/requests',
      { ...input, type: input.type ?? 'P2P' },
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async cancelPaymentRequest(
    ...
    return this.client.post<{}, PaymentRequestActionResult>(
      `/payments/requests/${encodeURIComponent(requestId)}/cancel`,
      {},
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async declinePaymentRequest(
    ...
    return this.client.post<{}, PaymentRequestActionResult>(
      `/payments/requests/${encodeURIComponent(requestId)}/decline`,
      {},
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async remindPaymentRequest(
    ...
    return this.client.post<{}, PaymentRequestActionResult>(
      `/payments/requests/${encodeURIComponent(requestId)}/remind`,
      {},
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
...
export class TransactionHistoryService extends BaseService {
  ...
  async getHistory(
    ...
    if (params.eventSubtype) search.set('eventSubtype', params.eventSubtype);
    ...
    if (typeof params.excludeUserOperationWrappers === 'boolean') {
      search.set(
        ...
        String(params.excludeUserOperationWrappers),
      );
    }
    ...
    if (params.view && params.view !== 'grouped') search.set('view', params.view);
    ...
    return this.client.get<TransactionHistoryResponse>(path);
  }
  ...
  async getInstallationFailures(params: {
    ...
    const path = `/transactions/installations/${encodeURIComponent(params.installationId)}/failures${
      search.toString() ? `?${search.toString()}` : ''
    }`;
    return this.client.get<AgentInstallationFailuresResponse>(path);
  }
   ...
   * @scope payments:read
   */
  async getInstallationFailureCount(
    installationId: string,
  ): Promise<AgentInstallationFailureCountResponse> {
    const path = `/transactions/installations/${encodeURIComponent(installationId)}/failures/count`;
    return this.client.get<AgentInstallationFailureCountResponse>(path);
  }
   ...
   * @scope payments:read
   */
  async getInstallationFailureCounts(
    installationIds: string[],
  ): Promise<AgentInstallationFailureCountsResponse> {
    const search = new URLSearchParams();
    ...
    const path = `/transactions/installations/failures/counts${
      search.toString() ? `?${search.toString()}` : ''
    }`;
    return this.client.get<AgentInstallationFailureCountsResponse>(path);
  }
}
...
export const formatTransactionAmount = (
  item: Pick<
    TransactionHistoryItem,
    'amountRaw' | 'tokenDecimals' | 'tokenSymbol' | 'direction'
  >,
...
export class MarketplaceCatalogService extends BaseService {
  ...
  async listAgents(): Promise<MarketplaceAgent[]> {
    ...
    const raw = await this.client.get<MarketplaceCatalogResponseDto[]>(
      '/marketplace/tenant/catalog',
    );
   ...
   * @scope agents:read
   */
  async listTenantAgents(): Promise<MarketplaceAgent[]> {
    const raw = await this.client.get<MarketplaceCatalogResponseDto[]>(
      '/marketplace/tenant/catalog',
    );
    return (raw ?? []).map(mapMarketplaceAgent);
...
/** Raw API response for marketplace catalog. */
interface MarketplaceCatalogResponseDto {
  id: string;
  ...
  install_questions?: AgentInstallQuestion[] | null;
  install_questions_version?: number | null;
  network?: {
    id?: string | null;
    network_id: number | null;
    name: string;
    native_currency?: { symbol: string; name: string; decimals: number };
    is_testnet?: boolean;
  } | null;
  operating_token?: {
  ...
  available_operating_tokens?: Array<{
    token_pool_id: string;
    symbol: string;
    name: string;
    contract: string;
  ...
  fee_summary?: {
    ...
    tenant_fee_bps: number;
    total_fee_bps: number;
    developer_remainder_bps: number;
  } | null;
  minimum_budget?: string | null;
  minimum_budget_by_token_pool_id?: Record<string, string> | null;
  min_validity_days?: number | null;
 ...
 */
function mapMarketplaceAgent(
  dto: MarketplaceCatalogResponseDto,
): MarketplaceAgent {
  return {
    id: dto.id,
    ...
    network: dto.network
      ? {
          networkId:
            dto.network.network_id !== undefined &&
    ...
    availableOperatingTokens: Array.isArray(dto.available_operating_tokens)
      ...
      : null,
    category: dto.category as MarketplaceAgent['category'],
    pricingModel: dto.pricing_model as MarketplaceAgent['pricingModel'],
    ...
    status: dto.status as MarketplaceAgent['status'],
...
export class AgentActionsService extends BaseService {
   ...
   * @scope agents:read
   */
  async listInstallations(): Promise<AgentInstallationDetails[]> {
    const raw =
      await this.client.get<AgentInstallationDetailsResponseDto[]>('/agents');
  ...
  async getInstallation(
    agentActorId: string,
  ): Promise<AgentInstallationDetails> {
    const raw = await this.client.get<AgentInstallationDetailsResponseDto>(
      `/agents/${encodeURIComponent(agentActorId)}`,
    );
  ...
  async prepareInstallation(
    ...
    const selectedTokenPoolId = String(
      request.selectedTokenPoolId ?? '',
    ).trim();
    const tokenBudgetModeRaw =
      typeof request.tokenBudgetMode === 'string'
        ? request.tokenBudgetMode.trim().toLowerCase()
        : '';
    const tokenBudgetMode =
      tokenBudgetModeRaw === 'single' || tokenBudgetModeRaw === 'all'
        ? tokenBudgetModeRaw
        : '';
    if (tokenBudgetModeRaw && !tokenBudgetMode) {
      throw new ValidationError(
        "tokenBudgetMode must be either 'single' or 'all'",
      );
    }
    const tokenBudgetsByTokenPoolId = Object.entries(
      request.tokenBudgetsByTokenPoolId ?? {},
    ).reduce<Record<string, string>>((acc, [rawTokenPoolId, rawBudget]) => {
      const tokenPoolId = String(rawTokenPoolId ?? '').trim();
      const budget = String(rawBudget ?? '').trim();
      if (!tokenPoolId) {
        throw new ValidationError(
          'tokenBudgetsByTokenPoolId keys must be non-empty token pool ids',
        );
      }
      if (!/^\d+$/.test(budget)) {
        throw new ValidationError(
          `tokenBudgetsByTokenPoolId.${tokenPoolId} must be a base-unit numeric string`,
        );
      }
    ...
    if (
      ...
      throw new ValidationError(
        'selectedTokenPoolId is required when tokenBudgetMode is single and token budgets are provided',
      );
    }
    ...
    if (request.validateInstallInputs) {
      if (!Array.isArray(request.installQuestions)) {
        throw new ValidationError(
          'installQuestions is required when validateInstallInputs is true',
        );
      }
      const validation = normalizeInstallInputs(
        request.installQuestions,
        request.installInputs ?? {},
        { mode: 'install' },
      );
      if (Object.keys(validation.errors).length > 0) {
        throw new ValidationError(
          `installInputs validation failed: ${JSON.stringify(
            validation.errors,
          )}`,
        );
      }
    ...
    const raw = await this.client.post<
      typeof payload,
      PrepareInstallationResponseDto
    >('/agents/install/prepare', payload, {
      headers: { 'X-XKOVA-IEE-Receipt': approval.receipt },
    });
    return {
  ...
  async confirmInstallation(
    ...
    const explicitReceipt =
      typeof request.receipt === 'string' ? request.receipt.trim() : '';
    let agentServiceId =
      typeof (request as any).agentServiceId === 'string'
        ? (request as any).agentServiceId.trim()
        : '';
    let budget =
      typeof (request as any).budget === 'string'
        ? (request as any).budget.trim()
        : '';
    const providedTxHash = String(request.transactionHash ?? '').trim();
    if (!providedTxHash) {
      throw new ValidationError(
        'transactionHash is required to confirm installation',
      );
    }
    const payload = {
    ...
    let approval: IeeReceiptApproval;
    if (explicitReceipt) {
      approval = {
        status: 'approved',
      ...
      if (!agentServiceId || !budget) {
        const resolved = await this.resolveInstallationById(
          request.installationId,
        );
        if (resolved) {
          let resolvedBudgetTotal = '';
          if (
            resolved.tokenBudgetsByTokenPoolId &&
            typeof resolved.tokenBudgetsByTokenPoolId === 'object'
          ) {
            let total = 0n;
            let hasBudget = false;
            for (const rawBudget of Object.values(
              resolved.tokenBudgetsByTokenPoolId,
            )) {
              const budgetValue = String(rawBudget ?? '').trim();
              if (!/^\d+$/.test(budgetValue)) continue;
      ...
      if (!agentServiceId || !budget) {
        throw new ValidationError(
          'agentServiceId and budget are required to confirm installation without an explicit receipt',
        );
      }
    ...
    if (
      approvalInstallationId &&
      approvalInstallationId !== request.installationId
    ) {
      throw new ValidationError(
        'installationId does not match SafeApprove approval',
      );
    }
    const approvalTxHash = String(approval.transactionHash ?? '').trim();
    if (
    ...
    const raw = await this.client.post<
      typeof payload,
      ConfirmInstallationResponseDto
    >('/agents/install/confirm', payload, {
      headers: { 'X-XKOVA-IEE-Receipt': approval.receipt },
      requestPolicy: {
        timeoutMs: 60_000,
    ...
    return {
      ...
      agentPass: raw.agent_pass,
      status: raw.status as ConfirmInstallationResponse['status'],
    };
  }
  ...
  async uninstallAgent(
    ...
    if (!installationId) {
      throw new ValidationError(
        'installationId is required to uninstall an agent',
      );
    }
    const approval = await this.ensureIeeReceipt({
    ...
    const raw = await this.client.delete<{
      status: string;
      installation_id?: string;
      unsigned_transaction?: unknown;
      operation_token?: string;
      thirdweb_client_id?: string;
    }>(`/agents/${encodeURIComponent(actorId)}`, {
      headers: { 'X-XKOVA-IEE-Receipt': approval.receipt },
    });
    ...
    if (raw.status === 'pending_signature') {
      if (!raw.unsigned_transaction || !raw.thirdweb_client_id) {
        throw new BadResponseError(
          'Uninstall response missing pending_signature fields',
        );
      }
      return {
  ...
  async confirmRevocation(
    ...
    if (!normalizedTxHash) {
      throw new ValidationError(
        'transactionHash is required to confirm revocation',
      );
    }
    ...
    return this.client.post<typeof payload, { status: 'revoked' }>(
      `/agents/installations/${encodeURIComponent(installationId)}/confirm-revocation`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async pauseInstallation(
    ...
    const raw = await this.client.post<
      typeof payload,
      { status: string; pause_code: string | null }
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/pause`,
      payload as any,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  ...
  async getInstallationStatus(installationId: string): Promise<{
    ...
    const raw = await this.client.get<{
      ...
      installation_id: string;
      created_at: string;
    }>(`/agents/installations/${encodeURIComponent(installationId)}/status`);
    return {
      status: raw.status,
      message: raw.message,
   ...
   * @scope agents:manage
   */
  async retryProvisioningWebhook(
    installationId: string,
    receipt?: string | null,
    ...
    return this.client.post<void, { success: boolean; message: string }>(
      `/agents/installations/${encodeURIComponent(installationId)}/retry-webhook`,
      undefined as any,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  async updateInstallationConfig(
    ...
    let resolvedInputs: Record<string, string | number> = installInputs;
    ...
    const tokenBudgetsByTokenPoolIdRaw =
      options?.tokenBudgetsByTokenPoolId !== undefined &&
      options?.tokenBudgetsByTokenPoolId !== null
        ? options.tokenBudgetsByTokenPoolId
        : null;
    let tokenBudgetsByTokenPoolId: Record<string, string> | null = null;
    if (tokenBudgetsByTokenPoolIdRaw !== null) {
      if (
        typeof tokenBudgetsByTokenPoolIdRaw !== 'object' ||
        Array.isArray(tokenBudgetsByTokenPoolIdRaw)
      ) {
        throw new ValidationError(
          'tokenBudgetsByTokenPoolId must be an object when provided',
        );
      }
      tokenBudgetsByTokenPoolId = Object.entries(
        tokenBudgetsByTokenPoolIdRaw,
      ).reduce<Record<string, string>>((acc, [rawTokenPoolId, rawBudget]) => {
        const tokenPoolId = String(rawTokenPoolId ?? '').trim();
        const budget = String(rawBudget ?? '').trim();
        if (!tokenPoolId) {
        ...
        if (BigInt(budget) <= 0n) {
          throw new ValidationError(
            `tokenBudgetsByTokenPoolId.${tokenPoolId} must be greater than 0`,
          );
        }
        acc[tokenPoolId] = budget;
        return acc;
    ...
    const tokenBudgetModeRaw =
      typeof options?.tokenBudgetMode === 'string'
    ...
    const tokenBudgetMode: 'single' | 'all' | null =
      tokenBudgetModeRaw === 'single' || tokenBudgetModeRaw === 'all'
        ? tokenBudgetModeRaw
        : null;
    ...
    const raw = await this.client.patch<
      typeof payload,
      {
        installation_id: string;
        install_inputs: Record<string, string>;
        install_questions_version: number | null;
        updated_at: string;
      }
    >(`/agents/installations/${encodeURIComponent(id)}/config`, payload, {
      headers: { 'X-XKOVA-IEE-Receipt': approval.receipt },
    });
    return {
      installationId: raw.installation_id,
  ...
  async increaseBudget(
    ...
    const raw = await this.client.patch<
      typeof payload,
      { updated: boolean; new_budget: string }
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/budget`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  ...
  async prepareIncreaseBudget(
    ...
    const raw = await this.client.post<
      typeof payload,
      PrepareIncreaseBudgetResponseDto
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/budget/prepare`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  ...
  async confirmIncreaseBudget(
    ...
    const receipt =
      typeof receiptOrOptions === 'string'
        ? receiptOrOptions
    ...
    const tokenBudgetMode: 'single' | 'all' | null =
      tokenBudgetModeRaw === 'single' || tokenBudgetModeRaw === 'all'
        ? tokenBudgetModeRaw
        : null;
    const normalizedAdditionalBudget =
      typeof additionalBudget === 'string' ? additionalBudget.trim() : '';
    const normalizedReceipt = typeof receipt === 'string' ? receipt.trim() : '';
    if (!normalizedReceipt && !normalizedAdditionalBudget) {
      throw new ValidationError(
        'additionalBudget is required to confirm a budget increase without a receipt',
      );
    }
    const normalizedTxHash = String(transactionHash ?? '').trim();
    if (!normalizedTxHash) {
      throw new ValidationError(
        'transactionHash is required to confirm a budget increase',
      );
    }
    const payload = {
    ...
    let approval: IeeReceiptApproval | null = null;
    ...
    if (approval) {
      ...
      if (approvalToken && approvalToken !== preparationToken) {
        throw new ValidationError(
          'preparationToken does not match SafeApprove approval',
        );
      }
      const approvalTxHash = String(approval.transactionHash ?? '').trim();
      if (
    ...
    const raw = await this.client.post<
      ...
      { updated: boolean; new_budget: string }
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/budget/confirm`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approvalReceipt } },
    );
  ...
  async prepareDecreaseBudget(
    ...
    const raw = await this.client.post<
      ...
      PrepareIncreaseBudgetResponseDto
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/budget/decrease/prepare`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  ...
  async confirmDecreaseBudget(
    ...
    const tokenBudgetMode: 'single' | 'all' | null =
      tokenBudgetModeRaw === 'single' || tokenBudgetModeRaw === 'all'
        ? tokenBudgetModeRaw
        : null;
    const normalizedDecreaseAmount =
      typeof decreaseAmount === 'string' ? decreaseAmount.trim() : '';
    const normalizedReceipt = typeof receipt === 'string' ? receipt.trim() : '';
    if (!normalizedReceipt && !normalizedDecreaseAmount) {
      throw new ValidationError(
        'decreaseAmount is required to confirm a budget decrease without a receipt',
      );
    }
    const normalizedTxHash = String(transactionHash ?? '').trim();
    if (!normalizedTxHash) {
      throw new ValidationError(
        'transactionHash is required to confirm a budget decrease',
      );
    }
    const payload = {
      preparation_token: preparationToken,
    ...
    const raw = await this.client.post<
      ...
      { updated: boolean; new_budget: string }
    >(
      `/agents/installations/${encodeURIComponent(installationId)}/budget/decrease/confirm`,
      payload,
      { headers: { 'X-XKOVA-IEE-Receipt': approvalReceipt } },
    );
  ...
  async resumeInstallation(
    ...
    return this.client.post<undefined, { status: string }>(
      `/agents/installations/${encodeURIComponent(installationId)}/resume`,
      undefined as any,
      { headers: { 'X-XKOVA-IEE-Receipt': approval.receipt } },
    );
  }
  ...
  private async resolveInstallationById(
    installationId: string,
  ): Promise<AgentInstallationDetails | null> {
    const id = String(installationId ?? '').trim();
    if (!id) {
      return null;
    }
    ...
    return (
      installations.find(
        (installation) => installation.installationId === id,
      ) ?? null
    );
  }
}

/** Raw API response for agent installation details. */
interface AgentInstallationDetailsResponseDto {
  installation_id: string;
  ...
  fee_schedule?: {
    ...
    bound_at: string;
    source: {
      tenant_fee_config_updated_at: string | null;
  ...
  token_symbols_by_token_pool_id?: Record<string, string> | null;
  token_budgets_by_token_pool_id?: Record<string, string> | null;
  token_budget_used_by_token_pool_id?: Record<string, string> | null;
  transaction_count_by_token_pool_id?: Record<string, number> | null;
  token_budget_mode?: "single" | "all" | null;
  install_inputs?: Record<string, string> | null;
  install_questions?: AgentInstallQuestion[] | null;
  install_questions_version?: number | null;
  service: {
    name: string;
...
/** Raw API response for prepare installation. */
interface PrepareInstallationResponseDto {
  installation_id: string;
  agent_pass: string;
  account: string;
  unsigned_transaction: unknown;
...
interface PrepareIncreaseBudgetResponseDto {
...
/** Raw API response for confirm installation. */
interface ConfirmInstallationResponseDto {
  installation_id: string;
  agent_actor_id: string;
  agent_pass: string;
  status: string;
...
function mapAgentInstallationDetails(
  dto: AgentInstallationDetailsResponseDto,
): AgentInstallationDetails {
    ...
    ? dto.available_operating_tokens
        .map((token) => {
          ...
          const minimumBudgetRaw =
            typeof token?.minimum_budget === 'string'
          ...
          return {
            ...
            contract:
              typeof token?.contract === 'string' ? token.contract : null,
            decimals:
              Number.isFinite(decimalsRaw) && Number.isInteger(decimalsRaw)
                ? Number(decimalsRaw)
                : null,
            isStable:
              typeof token?.is_stable === 'boolean' ? token.is_stable : null,
            networkPoolId:
              typeof token?.network_pool_id === 'string'
                ? token.network_pool_id
                : null,
            logoUrl:
              typeof token?.logo_url === 'string' ? token.logo_url : null,
            minimumBudget:
              minimumBudgetRaw && /^\d+$/.test(minimumBudgetRaw)
                ? minimumBudgetRaw
                : null,
          };
        })
  ...
  const tokenBudgetsByTokenPoolIdRaw =
    ...
    !Array.isArray(dto.token_budgets_by_token_pool_id)
      ? dto.token_budgets_by_token_pool_id
      : null;
  const tokenSymbolsByTokenPoolIdRaw =
  ...
  const tokenSymbolsByTokenPoolId =
    tokenSymbolsByTokenPoolIdRaw &&
    Object.keys(tokenSymbolsByTokenPoolIdRaw).length > 0
      ? Object.entries(tokenSymbolsByTokenPoolIdRaw).reduce<
          Record<string, string>
        >((acc, [rawTokenPoolId, rawSymbol]) => {
          const tokenPoolId = String(rawTokenPoolId ?? '').trim();
  ...
  const tokenBudgetsByTokenPoolId =
    ...
    Object.keys(tokenBudgetsByTokenPoolIdRaw).length > 0
      ? Object.entries(tokenBudgetsByTokenPoolIdRaw).reduce<
          Record<string, string>
        >((acc, [rawTokenPoolId, rawBudget]) => {
          const tokenPoolId = String(rawTokenPoolId ?? '').trim();
          const budgetValue = String(rawBudget ?? '').trim();
      ...
      : null;
  const tokenBudgetUsedByTokenPoolIdRaw =
  ...
  const tokenBudgetUsedByTokenPoolId =
    tokenBudgetUsedByTokenPoolIdRaw &&
    Object.keys(tokenBudgetUsedByTokenPoolIdRaw).length > 0
      ? Object.entries(tokenBudgetUsedByTokenPoolIdRaw).reduce<
          Record<string, string>
        >((acc, [rawTokenPoolId, rawAmount]) => {
          const tokenPoolId = String(rawTokenPoolId ?? '').trim();
      ...
      : null;
  const tokenBudgetModeRaw =
    typeof dto.token_budget_mode === 'string'
  ...
  const tokenBudgetMode: 'single' | 'all' | null =
    tokenBudgetModeRaw === 'single' || tokenBudgetModeRaw === 'all'
      ? tokenBudgetModeRaw
      : null;
  const transactionCountByTokenPoolIdRaw =
  ...
  const transactionCountByTokenPoolId =
    transactionCountByTokenPoolIdRaw &&
    Object.keys(transactionCountByTokenPoolIdRaw).length > 0
      ? Object.entries(transactionCountByTokenPoolIdRaw).reduce<
          Record<string, number>
        >((acc, [rawTokenPoolId, rawCount]) => {
          const tokenPoolId = String(rawTokenPoolId ?? '').trim();
          const count =
  ...
  return {
    ...
    agentServiceId: dto.agent_service_id,
    status: dto.status as AgentInstallationDetails['status'],
    ...
    feeSchedule: dto.fee_schedule
      ? {
          ...
          source: {
            tenantFeeConfigUpdatedAt:
              dto.fee_schedule.source?.tenant_fee_config_updated_at ?? null,
            updatedByUserId:
              dto.fee_schedule.source?.updated_by_user_id ?? null,
          },
        }
      : null,
    feeSummary: dto.fee_summary
