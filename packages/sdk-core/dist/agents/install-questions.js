const getSelectOptionValues = (question) => new Set((question.options ?? [])
 ...
 */
export function getInstallInputDefaults(questions) {
    const defaults = {};
 ...
 * - None.
 */
export function normalizeInstallInputs(questions, inputs, options = {}) {
    const errors = {};
    const normalized = {};
    const mode = options.mode ?? "install";
    const includeDefaults = options.includeDefaults ?? (mode === "install" ? true : false);
