...
import { UserProfileService } from "@xkova/sdk-core";
...
export const useUserProfile = () => {
    ...
    const service = useMemo(() => new UserProfileService({ client: authClient, iee }), [authClient, iee]);
    ...
    const updateAvatar = useCallback(async (file, options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        setIsLoading(true);
