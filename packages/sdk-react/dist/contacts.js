...
import { ContactsService, } from "@xkova/sdk-core";
...
export const useContacts = (filter) => {
    ...
    const [favoritesOnly, setFavoritesOnly] = useState(undefined);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const abortRef = useRef(null);
    const service = useMemo(() => new ContactsService({ client: apiClient, iee }), [apiClient, iee]);
...
export const useUpdateContact = () => {
    ...
    const scopes = state.tokens?.scope ?? [];
    const canManage = scopes.includes("all") || scopes.includes("contacts:manage");
    const update = useCallback(async (contactId, input, options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        if (!canManage) {
...
export const useDeleteContact = () => {
    ...
    const scopes = state.tokens?.scope ?? [];
    const canManage = scopes.includes("all") || scopes.includes("contacts:manage");
    const remove = useCallback(async (contactId, options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        if (!canManage) {
