...
import { buildSubmitSendPaymentBody, normalizePaymentRequestsList, normalizeSubmitSendPaymentInput, } from "@xkova/sdk-core";
...
export const useIncomingPaymentRequestHistory = (filter) => {
    ...
    const fetch = useCallback(async (options) => {
        ...
        try {
            ...
            if (filterOffset !== undefined)
                params.set("offset", String(filterOffset));
    ...
    useEffect(() => {
        if (!refreshMs || refreshMs <= 0 || state.status !== "authenticated")
            return;
 ...
 */
export const usePendingPaymentRequestsInbox = (options) => {
...
export const useCreatePaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const create = useCallback(async (params, options) => {
        if (state.status !== "authenticated") {
        ...
        try {
            ...
            const expiresAt = expiresAtRaw.length > 0
                ? expiresAtRaw
            ...
            const result = await apiClient.post(`/payments/requests`, body, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
...
export const useCancelPaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const cancel = useCallback(async (requestId, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/requests/${requestId}/cancel`, {}, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payment-requests");
            return result;
        }
        catch (err) {
...
export const useDeclinePaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const decline = useCallback(async (requestId, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/requests/${requestId}/decline`, {}, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payment-requests");
            return result;
        }
        catch (err) {
...
export const useCompletePaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const complete = useCallback(async (requestId, input, options) => {
        if (state.status !== "authenticated") {
        ...
        try {
            ...
            const result = await apiClient.post(`/payments/requests/${requestId}/complete`, body, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payment-requests");
            return result;
        }
        catch (err) {
...
export const usePayPendingPaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const pay = useCallback(async (request, options) => {
        if (state.status !== "authenticated") {
        ...
        try {
            ...
            const transactionType = String(options?.sendTransactionType ?? request?.transactionType ?? "").trim();
            if (!transactionType) {
            ...
            if (transactionType === "p2p_request" || transactionType === "store_request") {
                throw new Error("sendTransactionType must be a send/payment transaction type (not a request type)");
            }
            ...
            const expiresAt = expiresAtRaw.length > 0
                ? expiresAtRaw
                : new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
            const providedTxHash = typeof options?.sendTransactionHash === "string"
            ...
            const sendPayment = await apiClient.post("/payments/send", body, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            ...
            const completion = await complete(requestId, { transactionHash, network: networkId }, { receipt: options?.completeReceipt });
...
export const useRemindPaymentRequest = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const remind = useCallback(async (requestId, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/requests/${requestId}/remind`, {}, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payment-requests");
            return result;
        }
        catch (err) {
