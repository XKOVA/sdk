...
import { TransfersService, } from "@xkova/sdk-core";
 ...
 */
export const useTransferTransactions = (filter) => {
    ...
    const service = useMemo(() => new TransfersService({ client: apiClient, iee }), [apiClient, iee]);
    const fetch = useCallback(async () => {
        ...
        try {
            const response = await service.listTransferTransactions({
                type: filter?.type,
...
export const useCreateTransferTransaction = () => {
    ...
    const [error, setError] = useState(null);
    const service = useMemo(() => new TransfersService({ client: apiClient, iee }), [apiClient, iee]);
    const create = useCallback(async (input, options) => {
        if (state.status !== "authenticated") {
...
export const useExecuteFaucetTransfer = () => {
    ...
    const [error, setError] = useState(null);
    const service = useMemo(() => new TransfersService({ client: apiClient, iee }), [apiClient, iee]);
    const execute = useCallback(async (input, options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        setIsLoading(true);
...
export const useUpdateTransferTransaction = () => {
    ...
    const [error, setError] = useState(null);
    const service = useMemo(() => new TransfersService({ client: apiClient, iee }), [apiClient, iee]);
    const update = useCallback(async (transactionId, input, options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        setIsLoading(true);
...
export const useCreateTransferWidgetSession = () => {
    ...
    const create = useCallback(async (input) => {
        ...
        const authBaseUrl = normalizeTenantAuthBaseUrl(tenant?.authDomain ?? tenant?.auth_domain ?? null);
        if (!authBaseUrl) {
        ...
        try {
            ...
            if (!resp.ok) {
                const message = (json && (json.message || json.error)) ? String(json.message || json.error) : `Request failed (${resp.status})`;
            ...
            const expiresIn = typeof data.expires_in === "number"
                ...
                : typeof data.expiresIn === "number"
                    ? data.expiresIn
                    : 0;
            const widgetPath = typeof data.widget_path === "string"
                ? data.widget_path
...
export const useFaucets = (faucetsOnly = true) => {
    ...
    const faucets = useMemo(() => {
        const providers = (transferProviders || []);
        const mapped = [];
        for (const p of providers) {
            const isFaucet = p.integrationMethod === "faucet" || p.metadata?.faucetContract;
            if (faucetsOnly && !isFaucet)
                continue;
            const faucetContract = p.metadata?.faucetContract;
            if (!faucetContract)
                continue;
            const fallbackNetworkId = networks.find((n) => n?.networkId)?.networkId ?? null;
            const rawNetworkId = p.networkId ??
                p.supportedNetworks?.[0]?.networkId ??
                (fallbackNetworkId !== null ? Number(fallbackNetworkId) : null);
            const networkId = rawNetworkId !== null && Number.isFinite(Number(rawNetworkId))
                ? Number(rawNetworkId)
                : null;
            mapped.push({
