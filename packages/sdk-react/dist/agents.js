...
import { MarketplaceCatalogService, AgentActionsService, TransactionHistoryService, } from '@xkova/sdk-core';
import { useSDK } from './provider.js';
import { emitResourceUpdate, markSDKResourceFetched, subscribeResourceUpdate, useSDKResourceFreshness, } from './resources.js';
...
export const useAgentInstallationActions = () => {
    ...
    const prepareInstallation = useCallback(async (request) => {
        if (state.status !== 'authenticated') {
    ...
    const confirmInstallation = useCallback(async (request) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
    ...
    const updateInstallationConfig = useCallback(async (installationId, installInputs, options) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.updateInstallationConfig(installationId, installInputs, options);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const uninstallAgent = useCallback(async (agentActorId, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        ...
        try {
            ...
            const e = err instanceof Error ? err : new Error('Failed to uninstall agent');
            setError(e);
            throw e;
        }
        finally {
    ...
    const confirmRevocation = useCallback(async (installationId, transactionHash, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.confirmRevocation(installationId, transactionHash, receipt);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const increaseBudget = useCallback(async (installationId, additionalBudget, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.increaseBudget(installationId, additionalBudget, receipt);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
            const e = err instanceof Error ? err : new Error('Failed to increase budget');
            setError(e);
            throw e;
        }
        finally {
    ...
    const prepareIncreaseBudget = useCallback(async (installationId, additionalBudget, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            return await service.prepareIncreaseBudget(installationId, additionalBudget, receipt);
        }
        catch (err) {
            const e = err instanceof Error
                ? err
    ...
    const confirmIncreaseBudget = useCallback(async (installationId, preparationToken, transactionHash, receiptOrOptions) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.confirmIncreaseBudget(installationId, preparationToken, transactionHash, receiptOrOptions);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const prepareDecreaseBudget = useCallback(async (installationId, decreaseAmount, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            return await service.prepareDecreaseBudget(installationId, decreaseAmount, receipt);
        }
        catch (err) {
            const e = err instanceof Error
                ? err
    ...
    const confirmDecreaseBudget = useCallback(async (installationId, preparationToken, transactionHash, receiptOrOptions) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.confirmDecreaseBudget(installationId, preparationToken, transactionHash, receiptOrOptions);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const resumeInstallation = useCallback(async (installationId, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.resumeInstallation(installationId, receipt);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const pauseInstallation = useCallback(async (installationId, reason, options) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.pauseInstallation(installationId, reason, options);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
    ...
    const getInstallationStatus = useCallback(async (installationId) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        return service.getInstallationStatus(installationId);
    }, [state.status, service]);
    const retryProvisioningWebhook = useCallback(async (installationId, receipt) => {
        if (state.status !== 'authenticated') {
            throw new Error('User is not authenticated');
        }
        setIsLoading(true);
        setError(null);
        try {
            const result = await service.retryProvisioningWebhook(installationId, receipt);
            emitResourceUpdate('agent-installations');
            return result;
        }
        catch (err) {
