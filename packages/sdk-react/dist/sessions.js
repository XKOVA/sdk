...
import { SessionManagementService, } from "@xkova/sdk-core";
...
export const useUserSessions = (options) => {
    ...
    const service = useMemo(() => new SessionManagementService({ client: authClient, iee }), [authClient, iee]);
    const scopes = state.tokens?.scope ?? [];
    const canRead = scopes.includes("all") ||
        scopes.includes("account:read") ||
        scopes.includes("account:manage");
    const canManage = scopes.includes("all") ||
        scopes.includes("account:manage");
    const refetch = useCallback(async (params) => {
        if (state.status !== "authenticated")
            return;
        if (!canRead) {
            throw new Error("Insufficient scope. Required: account:read or account:manage (any)");
        }
        ...
        try {
            ...
            if (latestFetchRef.current !== token)
                return;
            setData(result);
            // Clamp the offset to the last valid page when the total shrinks.
            if (typeof result.total === "number" && result.total >= 0) {
                const maxOffset = result.total === 0 ? 0 : Math.floor((result.total - 1) / pageSize) * pageSize;
                if (offset > maxOffset) {
                    setOffset(maxOffset);
                }
            }
    ...
    const revokeSession = useCallback(async (sessionId, options) => {
        if (state.status !== "authenticated") {
    ...
    const revokeOtherSessions = useCallback(async (options) => {
        if (state.status !== "authenticated") {
            throw new Error("User is not authenticated");
        }
        if (!canManage) {
    ...
    return {
        ...
        hasNextPage: typeof data.total === "number" ? offset + pageSize < data.total : false,
