...
import { buildSubmitSendPaymentBody, normalizeSubmitSendPaymentInput, } from "@xkova/sdk-core";
...
export const useSubmitSendPayment = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const submit = useCallback(async (payload, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post("/payments/send", body, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
...
export const useCancelSendPayment = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const cancel = useCallback(async (paymentId, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/send/${paymentId}/cancel`, {}, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payments");
            return result;
        }
        catch (err) {
...
export const useRemindSendPayment = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const remind = useCallback(async (paymentId, options) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/send/${paymentId}/remind`, {}, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payments");
            return result;
        }
        catch (err) {
...
export const useVerifySendPaymentTransaction = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const verify = useCallback(async (paymentId, params, options) => {
        if (state.status !== "authenticated") {
        ...
        try {
            ...
            const result = await apiClient.post(`/payments/send/${paymentId}/verify`, payload, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payments");
            return result;
        }
        catch (err) {
...
export const useCancelPendingPaymentOnchain = () => {
    ...
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState(null);
    const cancelOnchain = useCallback(async (paymentId, params) => {
        if (state.status !== "authenticated") {
        ...
        setError(null);
        try {
            const result = await apiClient.post(`/payments/send/${paymentId}/pending-payment/cancel`, { cancelTxHash: resolvedCancelTxHash }, { headers: { "X-XKOVA-IEE-Receipt": approval.receipt } });
            emitResourceUpdate("payments");
            return result;
        }
        catch (err) {
 ...
 */
export const usePendingPaymentsContract = (networkId) => {
    const { networks, isLoading } = useTenantConfig();
    const targetNetworkId = networkId ?? networks?.[0]?.networkId;
    const normalizedNetworkId = targetNetworkId !== undefined && targetNetworkId !== null
        ? String(targetNetworkId)
        : null;
    const network = useMemo(() => normalizedNetworkId
        ? networks.find((n) => String(n?.networkId) === normalizedNetworkId)
        : undefined, [networks, normalizedNetworkId]);
    const pendingPaymentsContract = network?.pendingPaymentsContract ??
        network?.pending_payments_contract ??
        null;
    let error = null;
    if (!isLoading && normalizedNetworkId && !network) {
        ...
        error = new Error(`Pending payments contract not configured for network ${normalizedNetworkId ?? "default"}`);
    }
    const data = !isLoading && !error && pendingPaymentsContract
        ? {
