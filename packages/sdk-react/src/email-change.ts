...
import { resolveHostedEmailChangeUrl } from "@xkova/sdk-core";
 ...
 */
export const useHostedEmailChange = (options?: HostedEmailChangeOptions) => {
  ...
  const url = useMemo(() => {
    ...
    try {
      return resolveHostedEmailChangeUrl({
        authDomain,
  ...
  const launch = useCallback(
    (override?: { returnTo?: string }) => {
      try {
        ...
        const target = resolveHostedEmailChangeUrl({
          authDomain,
