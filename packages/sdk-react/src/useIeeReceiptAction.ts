...
import { type LaunchIeeResult } from "@xkova/sdk-browser";
 ...
 */
export function useIeeReceiptAction(params: { ieePath?: string } = {}) {
  const { authClient, oauth } = useSDK();
  const { tenantId, clientId, userId } = useIeeContext();
  const { tenant } = useTenantConfig();
  const [state, setState] = useState<IeeReceiptActionState>({ status: "idle" });
    ...
    async (options: {
      ...
      try {
        ...
        if (resolved.spec.thirdPartyUnsupported) {
          throw new IeeError(
            `Action is not supported for third-party clients (${resolved.spec.sdkActionType}).`,
            "THIRD_PARTY_ACTION_UNSUPPORTED",
            { sdkActionType: resolved.spec.sdkActionType, serverActionType: resolved.spec.serverActionType },
          );
        }
        ...
        if (launchResult.status === "approved") {
          const extras = launchResult as LaunchIeeResult & {
            contextHash?: string | null;
          ...
          const next = {
            ...
            installationId:
              extras.installationId ?? extras.installation_id ?? null,
            preparationToken:
              extras.preparationToken ?? extras.preparation_token ?? null,
            transactionHash: extras.transactionHash ?? null,
            resolvedPayload:
              (extras as any).resolvedPayload ??
              (extras as any).resolved_payload ??
              null,
          };
          setState(next);
          return next;
        ...
        if (launchResult.status === "cancelled") {
          const next = { status: "cancelled" as const };
          setState(next);
          return next;
        }
