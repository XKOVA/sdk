...
const buildCacheRequest = (cacheKey: string): Request =>
  new Request(`https://xkova.local/avatar-proxy/${encodeURIComponent(cacheKey)}`);
...
const deleteAvatarCache = async (cacheKey: string) => {
 ...
 */
export const useCachedProxyAvatarUrl = (sourceUrl: string | null) => {
  ...
  return useMemo(() => {
    if (!shouldProxy) return sourceUrl;
